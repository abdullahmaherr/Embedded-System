...
uint8_t HAL_KEYPAD_PressedKey(void)
{
	...
	MCAL_GPIO_PinInit(KEYPAD_ROW_PORT_ID,KEYPAD_ROW_FIRST_PIN_ID, GPIO_PIN_OUTPUT);
	MCAL_GPIO_PinInit(KEYPAD_ROW_PORT_ID,KEYPAD_ROW_FIRST_PIN_ID+1, GPIO_PIN_OUTPUT);
	MCAL_GPIO_PinInit(KEYPAD_ROW_PORT_ID,KEYPAD_ROW_FIRST_PIN_ID+2, GPIO_PIN_OUTPUT);
	MCAL_GPIO_PinInit(KEYPAD_ROW_PORT_ID,KEYPAD_ROW_FIRST_PIN_ID+3, GPIO_PIN_OUTPUT);
	...
	while(1)
	{
		for(row = 0; row < KEYPAD_NUM_ROW; row++)/* loop for rows */
		{
			...
			{
				if(MCAL_GPIO_ReadPin(KEYPAD_COL_PORT_ID, (KEYPAD_COL_FIRST_PIN_ID+col)) == 0)
				{
#if (KEYPAD_NUM_COL == 3)
